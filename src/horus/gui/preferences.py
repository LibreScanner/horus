...
class PreferencesDialog(wx.Dialog):
	def __init__(self, parent):
		...
		self.baudRateCombo = wx.ComboBox(self, choices=self.baudRates, size=(170,-1), style=wx.CB_READONLY)
		self.cameraIdLabel = wx.StaticText(self, label=_("Camera Id"))
		self.cameraIdNames = self.main.videoList()
		self.cameraIdCombo = wx.ComboBox(self, choices=self.cameraIdNames, size=(170,-1), style=wx.CB_READONLY)
	...
	def loadFirmware(self, hexBaudRate, clearEEPROM):
		avr_dude = AvrDude(port=self.serialNameCombo.GetValue(), baudRate=hexBaudRate)
		extraFlags = []
		if clearEEPROM:
			extraFlags = ["-D"]
		self.count = -50
