...
class validFloat(object):
    ...
    """
    Checks if the given value in the setting is a valid float.
    An invalid float is an error condition.
    ...
    def validate(self):
        try:
            ...
            return ERROR, '"' + str(self.setting.getValue()) + \
                '" is not a valid number or expression'


class validInt(object):

    """
    Checks if the given value in the setting is a valid integer.
    An invalid integer is an error condition.
    And supports a minimum and/or maximum value. The min/max values are error conditions.
    If the value == min or max then this is also an error.
    """
...
class wallThicknessValidator(object):
    ...
    """
    Special wall-thickness validator.
    The wall thickness is used to calculate the amount of shells and the thickness of the shells.
    But, on certain conditions the resulting wall-thickness is not really suitable for printing.
    The range in which this can happen is small.
    ...
    def validate(self):
        ...
        try:
            ...
            if wallThickness <= nozzleSize * 0.5:
                return ERROR,
                'Trying to print walls thinner then the half of your nozzle size, '
                'this will not produce anything usable'
            if wallThickness <= nozzleSize * 0.85:
                return WARNING,
                'Trying to print walls thinner then the 0.8 * nozzle size. '
                'Small chance that this will produce usable results'
            ...
            if lineWidth >= nozzleSize * 1.5 and lineWidthAlt <= nozzleSize * 0.85:
                ...
                'Current selected wall thickness results in a line thickness of ' + \
                    str(lineWidthAlt) + 'mm which is not recommended with your nozzle of ' + \
                    str(nozzleSize) + 'mm'
            return SUCCESS, ''
        except ValueError:
            # We already have an error by the int/float validator in this case.
...
class printSpeedValidator(object):
    ...
    def validate(self):
        ...
        try:
            ...
            maxPrintVolumePerSecond = 10 / \
                (math.pi * (0.2 * 0.2)) * (math.pi * (nozzleSize / 2 * nozzleSize / 2))

            if printVolumePerSecond > maxPrintVolumePerSecond:
                return WARNING,
                'You are trying to print more then %.1fmm^3 of filament per second. '
                'This might cause filament slipping. '
                '(You are printing at %0.1fmm^3 per second)' % \
